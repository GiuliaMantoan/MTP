function Res = QRboot_SM_covid(X, y, H, includeLagY, QQ, nDrawsBoot, nLagsBootVAR, covid, exo)
...
%     the periods t+1,...,t+h - 
%%    MODIFIED BY SM on may 2025 --> the dependent variable is y(t+h) - not the average value
...
%                  to 4.
%%    MODIFIED BY SM on may 2025 - added two extra input 
...
%        constructed outside the function (in the baseline version 1 for q2
%        and q3 2020)
%
%
% Output arguments:
...
%%    MODIFIED BY SM on may 2025 - add covid dummy
if covid == 1 
    Z = [ones(size(y)), X, exo];
elseif covid == 0
    Z = [ones(size(y)), X];
...
%%    MODIFIED BY SM on may 2025 -  pre-allocate B2 BQ if considering covid 
if covid == 1
    B2 = zeros(size(X,2)+2,size(H,2));
    BQ = zeros(size(X,2)+2,size(QQ,2), size(H,2));
end
...
%%    MODIFIED BY SM on may 2025 
    ...
    if covid == 1 && all( Z(1:end-h,end) == 0 ) 
    Zh = Z(1:end-h,1:end-1);
    end
    ...
    for jq = 1:length(QQ)
        ...
        bq = rq_fnm(Zh, yh, QQ(jq));
        BQ(1:size(Zh,2), jq, h) = bq';
...
if nDrawsBoot > 0
    ...
    DataBoot = VARsimul_covid([y, X], nLagsBootVAR, nDrawsBoot, covid, exo);
    DataBoot = DataBoot(1:length(y), :, :);

    %%    MODIFIED BY SM on may 2025 
    if covid == 1
        bB2 = zeros(size(X,2)+2,size(H,2),nDrawsBoot);
        bBQ = zeros(size(X,2)+2,size(QQ,2), size(H,2),nDrawsBoot);
    end
    ...
    for jm = 1:nDrawsBoot
        ...
        for h = H
            %%    MODIFIED BY SM on may 2025 
            Yh(:, h) = y; % filter(ones(1, h) / h, 1, y); filter(ones(1, h) / h, 1, y);
            % Yh(1:(h - 1), h) = NaN;
        end
        ...
        %%    MODIFIED BY SM on may 2025 
        if covid == 1  
            Z = [ones(size(y)) DataBoot(:,2:end,jm), exo];
        elseif covid == 0
        ...
        for h = H
            ...
            % OLS regression of yh on Zh
            b2 = Zh \ yh;
            bB2(1:size(Zh,2), h, jm) = b2;
            for jq = 1:length(QQ)
                % Quantile regression of yh on Zh (for the jqth quantile)
                bq = rq_fnm(Zh, yh, QQ(jq));
                bBQ(1:size(Zh,2), jq, h, jm) = bq'; % store coefficients
            end
        end
    end
