function Res = QRboot_SM(X, y, H, includeLagY, QQ, nDrawsBoot, nLagsBootVAR)
...
%     the periods t+1,...,t+h - MODIFIED BY SM on may 2025 --> the
%     dependent variable is y, not the average value of y.
...
for h = H 
    ...
    Yh(:, h) = y; % filter(ones(1, h) / h, 1, y); % modified by Simone Maso May 2025
...
for jm = 1:nDrawsBoot
    ...
    y = DataBoot(:, 1, jm);
    for h = H
        Yh(:, h) = y; % filter(ones(1, h) / h, 1, y); filter(ones(1, h) / h, 1, y); % modified by Simone Maso May 2025
        % Yh(1:(h - 1), h) = NaN;
    end
