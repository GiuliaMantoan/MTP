...
%%
%%  COVID TREATMENT
%%    Quarters Q1 2020 – Q4 2021 are excluded from the estimation sample
%%    entirely (both Y and X rows are dropped).  No dummy variable is used.
%%    Set cfg.covidStart / cfg.covidEnd to adjust the exclusion window.
%%
...
%%    h=1 = nowcast; h=2..13 = 1..12 quarters ahead.
%%
%%  REQUIRES: Statistics and Machine Learning Toolbox (wishrnd)
%%            dataloading_quarterly.m  (intermediate_codes folder)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
figDir   = fullfile(outDir, 'figures_gdp');
...
cfg.covidEnd   = datenum(2022, 3, 31);   % Q1 2022 (annualised rate still spans Covid)

% Minnesota prior hyperparameters
cfg.bvar.p       = 1;     % VAR lag order  (1 recommended for quarterly data)
cfg.bvar.lambda1 = 0.2;   % overall tightness
cfg.bvar.lambda3 = 1;     % lag decay exponent
cfg.bvar.lambda4 = 1e3;   % constant prior scale (large = diffuse)
cfg.bvar.delta   = ones(numel(cfg.varnames), 1);  % own-lag prior mean
%   Set delta(n) = 0 for growth-rate / stationary variables.

% Posterior Monte Carlo
cfg.bvar.nDraws = 2000;   % draws per origin  *** SET >= 5000 FOR PRODUCTION ***

% Forecast settings
cfg.horizons  = 13;             % h=1 nowcast; h=2..13 → 1..12 quarters ahead
cfg.quantiles = 0.05:0.05:0.95;

% Plotting
cfg.hPlot = [2, 5, 9];   % horizons shown in rolling fan and forward fan (1Q, 4Q, 8Q)
...
fullFileName = dataFile;       covid        = 0;   % suppress covid in dataloading
dummyvarname = {'covid'};

dataloading_quarterly;   % produces countryData and dateNumeric
...
%    Covid exclusion : rows with date in [covidStart, covidEnd] are dropped
%                      from both Y and X before estimation
%    Forecast target : column 1 of Y_all              [dep var = g4rgdp]
%    h=1             : nowcast (period t = origin_abs)
%    h=2..H          : 1..H-1 quarters ahead
%
%  Steps per origin:
%    (a) Slice Y_raw; remove NaN rows and Covid rows → Y_est
%    (b) Build X_rhs from Y_est lags
%    (c) Construct Minnesota dummy observations
%    (d) OLS on augmented system → B_hat, S_post, nu_post
%    (e) Draw (Sigma, B) from Normal-IW posterior
%    (f) Simulate H-step path → pred_q(o, :, h)

% Pre-compute Covid exclusion mask for the full date vector
covid_mask_full = dateNumeric(:) >= cfg.covidStart & ...
                  dateNumeric(:) <= cfg.covidEnd;   % T_full × 1 logical

fprintf('BVAR GDP: running %d origins...\n', nOrigins);
tic;

for o = 1:nOrigins

    origin_abs = idx_est + o - 1;
    T_full_est = origin_abs - 1;

    %% ── Build clean estimation sample (drop NaN and Covid rows) ─────────
    Y_raw  = Y_all(1:T_full_est, :);
    cov_ex = covid_mask_full(1:T_full_est);          % Covid rows to exclude
    ok     = all(~isnan(Y_raw), 2) & ~cov_ex;        % keep: non-NaN, non-Covid
    Y_est  = Y_raw(ok, :);
    T_est  = size(Y_est, 1);

    if T_est <= p + K + 2, continue; end
    ...
    % Dummy 1  (N*p rows) — lag coefficients
    Yd1 = zeros(N*p, N);
    Xd1 = zeros(N*p, K);
    for l = 1:p
        for j = 1:N
    ...
    % Dummy 2  (N rows) — error covariance
    Yd2 = diag(sig);
    Xd2 = zeros(N, K);

    % Dummy 3  (1 row) — diffuse constant
    Yd3    = zeros(1, N);
    Xd3    = zeros(1, K);
    Xd3(1) = 1 / lam4;
    ...
    XXi     = (Xstar' * Xstar) \ eye(K);
    B_hat   = XXi * (Xstar' * Ystar);   % K×N
    E_aug   = Ystar - Xstar * B_hat;
    S_post  = E_aug' * E_aug;           % N×N
    nu_post = size(Ystar, 1) - K;

    if nu_post <= N + 1, continue; end

    %% ── Pre-compute Cholesky factors ─────────────────────────────────────
    [L_Omega, flag_Om] = chol(XXi, 'lower');   % K×K
    if flag_Om > 0, continue; end              % XXi not PD — skip
    S_inv = S_post \ eye(N);                   % for wishrnd

    %% ── Last p rows of Y_est for forecast initialisation ────────────────
    Y_init = Y_est(end-p+1:end, :);            % p×N

    %% ── Monte Carlo predictive draws ─────────────────────────────────────
    %
    %  Numerically stable IW draw:
    %    W ~ Wishart(S_inv, nu_post),  W = L_W * L_W'
    %    U = inv(L_W')  (upper triangular),  U*U' = Sigma = inv(W)
    %    N(0,Sigma) draws: U * randn(N,1)
    ...
    for d = 1:nD
        ...
        U   = L_W' \ eye(N);              % upper triangular factor of Sigma
...
quarters_est = datetime(dateNumeric_est, 'ConvertFrom','datenum');
actualDT     = datetime(dateNumeric,     'ConvertFrom','datenum');
actual_var   = Y_all(:, 1);   % full dep-var series

% Covid window as datetime (for grey shading on rolling fan)
...
for ih = 1:numel(cfg.hPlot)
    ...
    tgt_dates = quarters_est + calquarters(h - 1);
    ...
    % Grey Covid rectangle (pushed behind data)
    yl = ylim(ax);
    covid_rect = fill(ax, ...
        [covid_start_dt, covid_end_dt, covid_end_dt, covid_start_dt], ...
        [yl(1), yl(1), yl(2), yl(2)], ...
        [0.75 0.75 0.75], 'EdgeColor','none', 'FaceAlpha',0.4, ...
        'DisplayName','Covid exclusion window');
    uistack(covid_rect, 'bottom');
    ylim(ax, yl);

    legend(ax,'show','Location','northoutside','Orientation','horizontal'); legend boxoff;
    set(ax,'FontSize',14);
    title(ax, sprintf('GDP (YoY) — BVAR rolling fan  |  h = %d quarters ahead', h-1));
    saveFig(fig, figDir, sprintf('BVAR_gdp_rolling_h%02d.png', h));
end

%% ── (b) Forward predictive density  (last origin) ────────────────────────
lastOrigDT = quarters_est(end);
n_fwd      = min(12, cfg.horizons - 1);
...
legend(ax,'show','Location','northoutside','Orientation','horizontal'); legend boxoff;
set(ax,'FontSize',13);
title(ax, sprintf('GDP (YoY) — BVAR forward fan  |  1–%d quarters ahead', n_fwd));
saveFig(fig, figDir, 'BVAR_gdp_forward_fan.png');
...
title(ax, 'BVAR GDP — Median (p50) by origin & horizon');
xlabel(ax, 'Forecast origin'); ylabel(ax, 'Horizon h (quarters)');
saveFig(fig, figDir, 'BVAR_gdp_median_heatmap.png');
