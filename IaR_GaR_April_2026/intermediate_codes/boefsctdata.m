...
opts.Sheet = 'CPI Forecast';

% Specify column names and types
opts.VariableTypes = repmat("double", 1, quarters_fcst + 1);

% Import the data
cpiinflprojectionparametersmpcS1 = readtable([cd '\Data\cpi_infl_projection_parameters_mpc.xlsx'], opts, "UseExcel", false);

% clear opts 
clear opts

% convert to array 
fcst_inf = table2array(cpiinflprojectionparametersmpcS1);
% fcst_inf = fcst_inf(~all(isnan(fcst_inf), 2), :);
...
% col number 
numCols = size(fcst_inf, 2);

% Extract every 10th row for each variable group using direct indexing

% MODE
idx_mode = 1:10:size(fcst_inf, 1);
mrkt_mode_raw = fcst_inf(idx_mode, :)';  
mrkt_mode = reshape(mrkt_mode_raw(~isnan(mrkt_mode_raw)), 13, []);

% MEAN.
idx_mean = 3:10:size(fcst_inf, 1);
mrkt_mean_raw = fcst_inf(idx_mean, :)';
mrkt_mean = reshape(mrkt_mean_raw(~isnan(mrkt_mean_raw)), 13, []);

% UNCERTAINTY (SIGMA)
idx_uncer = 4:10:size(fcst_inf, 1);
mrkt_uncer_raw = fcst_inf(idx_uncer, :)';
...
%clearvars -except mtestdata start_date end_date covid_date ctrynames varnames fullFileName momentlist outputFolder modelfcst
